...
class Verify {
     ...
     * @param string $key_app app传的key值
     * @return bool 用户验证码是否正确
     */
    public function check($code, $id = '' ,$key_app) {
        if($key_app){
//            file_put_contents('key_app.txt',$key_app);
//            file_put_contents('s_key_app.txt',S($key_app));
            if($this->authcode(strtoupper($code)) == S($key_app)) {
                S($key_app,null);
                return true;
            }
        }
...
//            file_put_contents('s_code1.txt',$this->authcode(strtoupper($code)).'---'.$secode['verify_code']);
//            file_put_contents('s_code.txt',$code);
     ...
     * @access public     
     * @param string $id 要生成验证码的标识     string $key
     * @return void
     */
    public function entry($id = '',$key_app='') {
        // 图片宽(px)
...
//        file_put_contents('other.txt',$code);
        if($key_app){
//            file_put_contents('other1.txt',$key_app);
            S($key_app,$code,60*1000);
//            file_put_contents('other2.txt',S($key_app));
        }
