...
//    }
    ...
    public function editPass(){
        if(empty($_POST)){
            ...
            if(empty($_POST['flag'])){
                $data = D('agency_earn')->create();
                if($data){
                    $where['e_id'] = session('E_ID');
                    $where['password'] = md5(I('post.old_password'));
                    $result = D('agency_earn')->where($where)->find();
                    if($result){
                        $data['password'] = md5($_POST['password']);
                        $data['utime'] = time();
                        $upd_res = D('agency_earn')->where($where)->limit(1)->save($data);
                        if($upd_res){
                            $this->success('修改密码成功,请退出重新登录！',U('Admin/editPass'));
                        }else{
                            $this->error('修改密码失败');
                        }
                    }else{
