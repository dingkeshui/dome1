 ...
 * 收入统计图
 */
class IncomeController extends AdminBasicController {
    ...
    public function income(){
        ...
        //横坐标赋值时间
        $x_res = $this->createX($start_time,$end_time,"平台流水(元)");
        //平台流水统计
    ...
    public function getSalesByDay($start_time,$end_time){
        //折线图数据 查询条件及对象
        $sales_line_p = array('title'=>'平台流水(元)','obj'=>D('OrderClass'),'flag'=>array());
        //数据参数
        ...
        //顶部文字subtitle
        $this->assign('day_date_flag','【日增加量(元)】　'.$start_time.'至'.$end_time);
    }
    ...
    public function getSalesByMonth($year){
        ...
        for($month = 1; $month <= 12; $month++){
            ...
            $field = "sum(total_price)";
            $amount = D('OrderClass')->where($where)->getField($field);
            $total =  sprintf("%.2f", $amount);
            $sales_line_data['平台流水(元)'][] = $total;
        }
        //创建折线
        $this->assign('month_line',$this->createLine($sales_line_data));
        //顶部文字subtitle
        $this->assign('month_date_flag','【月增加量(元)】　'.$year.'年');
    ...
    public function getLineData($start_date,$end_date,$parameter){
        ...
        for($i = 0; $i <= $day; $i++){
            foreach($parameter as $k => $value){
                ...
                $data[$k][] = $value['obj']->where($value['where'])->sum('total_price');
            }
        }
    ...
    public function getLineDataByMonth($year,$parameter){
        ...
        for($month = 1; $month <= 12; $month++){
            foreach($parameter as $k => $value){
                ...
                $value['where']['class_id'] = session('CLASS_E_ID');
                $data[$k][$month] = $value['obj']->where($value['where'])->sum('total_price');
            }
        }
        //添加标题
        foreach($parameter as $k => $value){
