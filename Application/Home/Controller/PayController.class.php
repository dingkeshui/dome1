        ...
        if (!empty($_POST['c_m_id'])){
            ...
            switch ($coupon['type']){
                case 1:
                    ...
                    $order_price = $_POST['order_price']-$coupon['min_price'];
                    break;
                case 2;
                    //折扣券
                    $data_log['coupon_money'] = sprintf("%.2f",(($coupon['money']/10))*$_POST['order_price']);;
                    $order_price = $_POST['order_price']-$data_log['coupon_money'];
                    break;
                case 3:
                    //满减券
    ...
    public function getHxShopOrderApi(){
        ...
        switch($_POST['type']){
            ...
            case 1:
                ...
                $order_res = M("ProductOrder")->where(['order_sn'=>$order_sn])->field("real_price,goods_gather,shop_id")->find();
                $order_price = $order_res['real_price'];
                //商户名称
        ...
        $CurrencyType = 156;
        //商户返回地址
        $MerchantUrl = "https://".$_SERVER['HTTP_HOST']."/index.php/Merchant/apporder/orderlist";
        //订单签名方式
        $RetEncodeType = 17;
        $BillExp = date('Y-m-d H:i:s',strtotime('+1year'));
        /************************************************************/
