...
class DeskModel extends Model {
     ...
     * 查询多条数据
     */
    public function selectDesk($where = array(),$order = '',$page_size = '',$parameter = array()){
     ...
     * 查询一条数据
     */
    public function findDesk($where){
        if(empty($where)){
            return false;
        }else{
            if($where['status'] == '' || empty($where['status'])){
     ...
     * 编辑数据
     */
    public function editDesk($where,$data){
