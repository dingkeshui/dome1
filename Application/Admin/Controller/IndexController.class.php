...
class IndexController extends AdminBasicController {
    ...
    public function left(){
        $where['a_id'] = session('A_ID');
        //判断是否是超级管理员是就把权限全部列出来
        if(session("A_ID") == 1){
            $list = D("AdminAction")->select();
            foreach($list as $k=>$v){
                $action_admin[$k] = $v['model']."/".$v['method'];
            }
        }else{
            $admin = D("Admin")->where($where)->find();
            $where['group_id'] = $admin['group_id'];
            $per = D("AdminGroup")->findAdminGroup($where);
            $action_admin = unserialize($per['permission']);
            foreach($action_admin as $k=>$v){
                $action = D("AdminAction")->where(array('action_id'=>$v))->find();
                $action_admin[$k] = $action['model']."/".$action['method'];
            }
        }
        $this->assign("action_admin",$action_admin);
        $this->display('left');
    }
