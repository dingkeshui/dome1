...
class IncomeController extends AdminBasicController {
    ...
    public function income(){
        ...
        //横坐标赋值时间
        $x_res = $this->createX($start_time,$end_time,"平台收入(元)");
        //平台收入统计
    ...
    public function getSalesByDay($start_time,$end_time){
        //折线图数据 查询条件及对象
        $sales_line_p = array('title'=>'平台收入(元)','obj'=>D('Price'),'flag'=>array());
        //数据参数
    ...
    public function getSalesByMonth($year){
        ...
        for($month = 1; $month <= 12; $month++){
            ...
            //平台收入
            $field = "sum(price)";
            $amount = D('Price')->where($where)->getField($field);
            $total =  sprintf("%.2f", $amount);
            $sales_line_data['平台收入(元)'][] = $total;
        }
        //创建折线
    ...
    public function createLine($data){
        ...
        foreach($data as $key => $value){
            $line_data = '';
            $line.= "{name: '".$key."',type:'line',smooth: true,data:[";
            foreach($value as $v){
                $line_data.=$v.',';
            }
