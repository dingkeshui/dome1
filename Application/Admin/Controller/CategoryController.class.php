...
class CategoryController extends AdminBasicController {
    ...
    public function editCategory(){
        if(IS_POST){
            ...
            if($data){
                ...
                if($res){
                    $this->success('编辑成功',U('Category/categoryList'));
                }else{
                    $this->error('编辑失败');
                }
            }else{
            ...
            $id = I('get.id');
            $category = M('Category')->where(array('cate_id'=>$id))->find();
    ...
    public function updateSort(){
        ...
        $data['utime'] = time();
        $res = M('Category')->where(array('cate_id'=>$_POST['cate_id']))->limit(1)->save($data);
        if($res){
            $this->ajaxReturn(1);
        }else{
            $this->ajaxReturn(0);
