    ...
    public function doLogin(){
        ...
        if(empty($_POST['account'])){
            ...
            if($find_res){
                ...
                if($find_res1){
                    //修改最后登录时间和ip 错误次数归零
                    $admin->editAdmin(array('shop_id'=>$find_res1['shop_id']),array('last_login_time'=>time(),'error_num'=>0,'login_ip'=>$_SERVER["REMOTE_ADDR"]));
                    ///设置session
                    session('SHOP_ACCOUNT',$find_res1['account']);
                    session('SHOP_ID',$find_res1['shop_id']);
                    session('SHOP_NAME',$find_res1['name']);
            ...
            if(!$this->check_verify($_POST['verify'])){
                ...
                if($find_res3){
                    //修改最后登录时间和ip 错误次数归零
                    $admin->editAdmin(array('shop_id'=>$find_res3['shop_id']),array('last_login_time'=>time(),'error_num'=>0,'login_ip'=>$_SERVER["REMOTE_ADDR"]));
                    ///设置session
                    session('SHOP_ACCOUNT',$find_res3['account']);
                    session('SHOP_ID',$find_res3['shop_id']);
                    session('SHOP_NAME',$find_res3['name']);
                    $this->ajaxMsg('success','登陆成功');
                }else{
