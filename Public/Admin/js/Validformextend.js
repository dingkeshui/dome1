...
//自定义校验
	$.extend($.Datatype,{
		"zh2-6": /^[\u4E00-\u9FA5\uf900-\ufa2d]{2,6}$/, //中文验证
		"zhd":/^[\u4E00-\u9FA5\uf900-\ufa2d]+$/,    //中文
		"nd": /^[a-zA-Z][a-zA-Z0-9_]{2,16}$/, //用户名名称
		//"ns6-18":/^[\w\.\s]{6,18}$/, //包括空格、制表符、换页符等等。等价于[ \f\n\r\t\v] [A-Za-z0-9_]
		"ns":/^[\w\.\s]$/, //包括空格、制表符、换页符等等。等价于[ \f\n\r\t\v] [A-Za-z0-9_]
		"w4-18":/^[\w]{4,18}$/,      //[A-Za-z0-9_]
		"v-1":/^\d*$/,      //等于-1
		"sname":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s\,\，]+$/,
		"tel":function(gets,obj,curform,regxp){
			...
			var reg1=regxp["m"],
				reg2=/(^((0[1,2,3]{1}\d{1}-?\d{8})|(0[3-9]{1}\d{2}-?\d{7,8}))$)|(^0?(13[0-9]|15[0-35-9]|18[0236789]|14[57])[0-9]{8}$)/,
		...
		"yyisValidBusCode":function(gets,obj,curform,datatype){//营业执照编号
			 if(yyisValidBusCode($.trim(gets))){
				 
				   return true;
			   }else{
				   return false;
			   }
				function yyisValidBusCode(busCode){
					//return true;
			        var ret=false;
	  ...
	  "validNum":function(gets,obj,curform,datatype){
		   if(validNum(obj,2,'','10000000')){
			   return true;
		   }else{
			   return false;
		   }
		  ...
		  function validNum(obj, pos, def, max) {
			  var val = $.trim($(obj).val());
				...
				if (!isNaN(val)) {
					...
					if (val.indexOf('.') == 0) {
						if (val.length == 1) {
							...
							var ffffss=parseInt(_ss, 10) != NaN ? def : '0' + val;
							$(obj).val(ffffss);
							if(ffffss==""){
								return false;
							}
				...
				if (__len == 1) {
					...
					var dd_dd_lendef=parseFloat(__arr[0]);
					$(obj).val(dd_dd_lendef);
					if(dd_dd_lendef==""){
						return false;
					}
				} else if (__len == 2) {
					if (__arr[0] != "") {
						...
						var dd_Strendef=parseInt(__arr[0]) + "." + __tStr;
						$(obj).val(dd_Strendef);
						if(dd_Strendef==""){
							return false;
						}
