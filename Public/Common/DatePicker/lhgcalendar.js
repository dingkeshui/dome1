...
J.exend( calendar.prototype, {
	...
	createbody: function(d,m){
		...
		txt = '<div id="py" title="上一年">&nbsp;</div><div id="pm" title="上一月">&nbsp;</div>' + txt + '<div id="ny" title="下一年">&nbsp;</div><div id="nm" title="下一月">&nbsp;</div>';
		
		J(chead).stcs({ width: '256px', height: '30px'}).html(txt); J(m).apch(chead);
		
		txt = '<table width="100%" cellspacing="0" cellpadding="0" border="0" height="200">' +
			  '<thead id="tdy"><tr><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td></tr></thead>' +
			  '<tbody id="idcal"></tbody></table>' +
			  '<div id="foot"><span id="t2">' + new Date().toLocaleDateString() + '</span><span id="t3">取消</span><span id="t1">今天</span></div>';
		
		var cbody = J(d).crte('div'); J(cbody).stcs('width','256px').html(txt); J(m).apch(cbody);
					...
					if(day){
						if( this.than && this.than == 'min' ){
						    if( this.y < this.ty || (this.y == this.ty && this.m < this.tm) || (this.y == this.ty && this.m == this.tm && day < this.td) ){
							    ...
							    J(cell).html(day).stcs('color','#ccc');
						}
