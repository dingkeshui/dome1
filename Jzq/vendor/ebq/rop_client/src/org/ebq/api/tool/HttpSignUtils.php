...
class HttpSignUtils{
	...
	static function checkHttpSign($bodyParams,$timestamp,$appKey,$appSecret,$sign,$expireLength=1800000){
		...
		$sign1=self::createHttpSign($bodyParams,$timestamp,$appKey,$appSecret);
     ...
     * @param $appKey
     * @param $appSecret
     * @return bool|null|string 返回请求结果
     * @throws Exception
     */
	static function doPostByObj($url,$requestObj,$appKey,$appSecret){
		if(is_null($url)){
			throw new Exception("url is null");
		}
		if(is_object($requestObj)&&is_subclass_of($requestObj,'org\ebq\api\model\RichServiceRequest')){
			if($requestObj->validate()){
				$requestArr=$requestObj->getObject2Array();
				return static::doPost($url,$requestArr,$appKey,$appSecret);
			}
		}else{
