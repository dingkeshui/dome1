...
use com\jzq\api\model\menu\CertiType;
...
class CertiAuthRequest extends RichServiceRequest{
	...
	function validate(){
        if($this->certiType!=CertiType::$CERTI_CLIENT&&$this->certiType!=CertiType::$CERTI_CLOUD){
            unset($this->certiType);
        }else{
            $this->certiType=static::trim($this->certiType);
        }
		if($this->applyTable!=null){
            if(!is_a($this->applyTable, 'org\ebq\api\model\bean\UploadFile')){
